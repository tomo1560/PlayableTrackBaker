...
全プローブの周期は GAIA の BPM 110 に同期しています（1拍≈0.545秒、4/4で1小節≈2.182秒）。
音楽のビートと絵のパルスのずれ自体が忠実度の指標になります。

| ステーション | 動き | 何を検証するか | 不忠実だとどう見えるか |
|---|---|---|---|
| Comet Circuit | 球がリサージュ8の字を描く(横1拍・縦0.5拍周期 ≈1.83/3.67Hz) | 30fpsサンプリングの高周波モーション追従 | エイリアシング、軌道のカクつき／過度な平滑化 |
| Drift Monolith | 微小振幅(0.002〜0.003)のドリフト(16小節)＋2拍ボブ＋隣に100倍振幅の分身 | High Precision Reduction 0.002の閾値付近の精度 | 微小動作の消失(本体だけ止まって見え、分身は動く) |
| Gyro Spinner | 1拍で1回転(660°/秒、30fpsで1フレーム22°)、ピッチは1小節1回転 | 四元数カーブのベイク精度 | 回転のエイリアシング、速度のブレ |
| Teleport Beacons | 1小節ごとに4スロットを瞬間移動(補間なし) | 離散ジャンプがベイクで補間されないか | ジャンプがスミア(滑らかに繋がって)見える |
| Orbit Pair (nested) | 親armが8小節で1回転、子counterが4小節で逆回転しつつ1小節周期で上下振動 | 深い階層のTransformパス記録 | 子だけ動きが欠落/親子の位相がズレる |
| Phase Choir | 8本のピラーが位相をずらして2拍周期で波打つ | 複数オブジェクト間のタイミング同期 | 波が揃わず乱れる、一部だけ遅延する |
| Scale Beat | 毎拍の鋭い指数アタック/ディケイでスケール | 非正弦(鋭い)エンベロープのキー削減耐性 | アタックが鈍る、拍とパルスがずれる |
